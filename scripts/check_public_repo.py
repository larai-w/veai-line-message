...
def main() -> int:
    ...
    for path in paths:
        try:
            ...
            # deletion is committed. They have no content left to inspect and must not make
            # a pre-commit/CI dry run fail while private material is being removed.
            if not args.staged and not (ROOT / path).exists():
                continue
            raise
